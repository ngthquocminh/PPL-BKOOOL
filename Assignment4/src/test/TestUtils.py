...
class TestCodeGen():
    ...
    def check(soldir,asttree,num):
        ...
        try:
            ...
            cmd_call = "java  -jar "+ JASMIN_JAR + " " + path + "/*.j"
            print(cmd_call)
            subprocess.call(cmd_call,shell=True,stderr=subprocess.STDOUT)
            cmd_call = "java -cp ./lib" + os.pathsep + ". BKoolClass"
            print(cmd_call)
            subprocess.run(cmd_call,shell=True, stdout = f, timeout=10)
