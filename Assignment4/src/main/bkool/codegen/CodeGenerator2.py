...
class CodeGenVisitor(BaseVisitor, Utils):
    ...
    def visitProgram(self, ast: Program, c):
        ...
        for x in ast.decl:
            if type(x) is FuncDecl:
                ...
                staticDecl = [Symbol(x.name.name.lower(), MType(partype, x.returnType),
                                     CName(self.className))] + staticDecl
            else:
    ...
    def visitVarDecl(self, ast: VarDecl, o: SubBody):
        ...
        self.emit.printout(self.emit.emitVAR(idx, varName, StupidUtils.retrieveType(varType), frame.getStartLabel(),
    ...
    def genMETHOD(self, decl: FuncDecl, o, frame: Frame):
        ...
        intype = [ArrayPointerType(StringType())] if isMain else [StupidUtils.retrieveType(x.varType) for x in
                                                                  decl.param]
        ...
        if isInit:
            self.emit.printout(
                self.emit.emitVAR(frame.getNewIndex(), "this", ClassType(self.className), frame.getStartLabel(),
                                  frame.getEndLabel(), frame))
        if isMain:
        ...
        if isClassInit:
            for x in self.listGlobalArray:
                ...
                self.emit.printout(
                    self.emit.emitInitNewStaticArray(self.className + "/" + x.variable.name, size, x.varType.eleType,
                                                     frame))
