...
class Emitter():
    ...
    *    @param typ the type of the constant
    ...
    def emitNEW(self, classname):
        
        return self.jvm.emitNEW(lexeme=classname)
    ...
    *   @param toLabel the ending label  of the scope where the variable is active.
    ...
    def emitINSTANCEFIELD(self, lexeme, typ, isFinal=False, value=None):
        isFinal = isFinal and (value is not None or type(typ) in [ClassType, ArrayType])
        emitType = self.getJVMType(typ)
        
        emit = self.jvm.emitINSTANCEFIELD(lexeme, self.getJVMType(typ))
        value = ("\""+value+"\"") if isinstance(value,str) and isinstance(typ,StringType) else value
        
        print("emitINSTANCEFIELD",emitType)
        if isFinal:
            if not "final" in emit:
                emit = reduce(
                    lambda e, w: e + " " + (("final "+w) if w==lexeme else w), 
                    emit.split(" ")[1:],
                    emit.split(" ")[0],
                ) # add final
            if not ("= "+str(value)) in emit and not type(typ) in [ClassType, ArrayType]:
                    emit = reduce(
                    lambda e, w: e + " " + (w.replace(w.strip(),w.strip()+" = "+str(value)) if w.strip()==emitType else w), 
                    emit.split(" ")[1:],
                    emit.split(" ")[0]
                ) # add = <value>
        return emit
    ...
    *   @param isFinal true in case of constant; false otherwise
    ...
    def emitATTRIBUTE(self, lexeme, in_, isFinal=False, value=None):
        ...
        #isFinal: Boolean
        #value: String
        isFinal = isFinal and (value is not None or type(in_) in [ClassType, ArrayType])
        emitType = self.getJVMType(in_)
        value = ("\""+value+"\"") if isinstance(value,str) and isinstance(in_,StringType) else value
        emit = self.jvm.emitSTATICFIELD(lexeme, emitType, isFinal)
        print("!!!!!",isFinal,emit)
        
        if isFinal:
            if not ("= "+str(value)) in emit and not type(in_) in [ClassType, ArrayType]:
                emit = reduce(
                    lambda e, w: e + " " + (w.replace(w.strip(),w.strip()+" = "+str(value)) if w.strip()==emitType else w), 
                    emit.split(" ")[1:],
                    emit.split(" ")[0],
    ...
    def emitGETSTATIC(self, lexeme, in_, frame,isFinal=False, value=None):
        #lexeme: String
        #in_: Type
        #frame: Frame
    ...
    *   @param in the type of the local array variable.
    ...
    def emitInitNewStaticArray(self, name, size, eleType, frame):
        ...
        if type(eleType) is StringType:
            result.append(self.jvm.emitANEWARRAY(Emitter.getFullType(eleType)))
        else:
            result.append(self.jvm.emitNEWARRAY(Emitter.getFullType(eleType)))
        if isinstance(name,str):
            result.append(self.jvm.emitPUTSTATIC(name, self.getJVMType(eleType)))
        return ''.join(result)
    ...
    *   .super java/lang/Object<p>
    ...
    def emitEPILOG(self):
        ...
        file.write(''.join([str(i) for i in self.buff]))
        file.close()
